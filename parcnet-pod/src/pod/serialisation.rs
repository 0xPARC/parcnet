...
pub fn null_ser<S>(s: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    s.serialize_none()
...
pub fn null_de<'de, D>(data: D) -> Result<(), D::Error>
where
...
pub fn fq_ser<S>(x: &Fq, s: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
...
pub fn fq_de<'de, D>(data: D) -> Result<Fq, D::Error>
where
    D: serde::de::Deserializer<'de>,
{
...
pub fn compressed_pt_ser<S>(pt: &Point, s: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
...
pub fn compressed_pt_de<'de, D>(data: D) -> Result<Point, D::Error>
where
    D: serde::de::Deserializer<'de>,
{
    let s: &str = serde::de::Deserialize::deserialize(data)?;
    let compressed_pt_bytes = general_purpose::STANDARD_NO_PAD
        .decode(s)
        .map_err(serde::de::Error::custom)?;
...
pub fn compressed_sig_ser<S>(sig: &Signature, s: S) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
...
pub fn compressed_sig_de<'de, D>(data: D) -> Result<Signature, D::Error>
where
    D: serde::de::Deserializer<'de>,
{
    let s: &str = serde::de::Deserialize::deserialize(data)?;
    let compressed_sig_bytes = general_purpose::STANDARD_NO_PAD
        .decode(s)
        .map_err(serde::de::Error::custom)?;
    decompress_signature(&array::from_fn(|i| compressed_sig_bytes[i]))
        .map_err(serde::de::Error::custom)
}
...
mod tests {
    ...
    fn serde_json_test() -> Result<(), Error> {
        let x = fq_de(&mut serde_json::Deserializer::from_str(
            "\"0x1d5ac1f31407018b7d413a4f52c8f74463b30e6ac2238220ad8b254de4eaa3a2\"",
        ))?;
        let y = fq_de(&mut serde_json::Deserializer::from_str(
        ...
        compressed_pt_ser(
            &example_pt,
            &mut serde_json::Serializer::new(&mut json_buffer),
        )?;
        let json_string = String::from_utf8(json_buffer)?;
        assert!(json_string == "\"xDP3ppa3qjpSJO+zmTuvDM2eku7O4MKaP2yCCKnoHZ4\"");
        let deserialised_example_pt =
            compressed_pt_de(&mut serde_json::Deserializer::from_str(&json_string))?;
