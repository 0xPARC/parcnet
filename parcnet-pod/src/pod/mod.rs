...
#[no_mangle]
pub extern "C" fn create_pod_ffi(
    private_key_ptr: *const u8,
    private_key_len: usize,

    // We’ll pass in a JSON string for the `data` to simplify bridging
    // Instead of messing with a complex struct array, we can let Rust parse JSON
    data_json_ptr: *const libc::c_char,

    // Output pointer for success/failure. Return 0 if success, 1 if error.
    out_error_code: *mut i32,

    // We can return a pointer to a JSON-serialized Pod if successful
) -> *mut libc::c_char {
    unsafe {
        // 1) Convert private_key from raw pointer
        let private_key = std::slice::from_raw_parts(private_key_ptr, private_key_len);

        // 2) Convert data_json
        if data_json_ptr.is_null() {
            *out_error_code = 1;
            return std::ptr::null_mut();
        }
        let c_str = std::ffi::CStr::from_ptr(data_json_ptr);
        let data_json = match c_str.to_str() {
            Ok(s) => s,
            Err(_) => {
                *out_error_code = 1;
                return std::ptr::null_mut();
        ...
        // 3) Parse data from JSON into Vec<(String, PodValue)>
        let parsed_data: Vec<(String, PodValue)> = match serde_json::from_str(data_json) {
            Ok(d) => d,
            Err(_) => {
                *out_error_code = 2;
                return std::ptr::null_mut();
        ...
        // 4) Call your internal create_pod
        let pod_result = create_pod(private_key, parsed_data);

        match pod_result {
            Ok(pod) => {
                // Serialize the Pod struct to JSON to send back to Go
                match serde_json::to_string(&pod) {
                    Ok(json_str) => {
                        *out_error_code = 0;
                        // Convert JSON string to a char* that Go can read
                        let c_string = std::ffi::CString::new(json_str).unwrap();
                        c_string.into_raw()
                    }
                    Err(_) => {
                        *out_error_code = 3;
