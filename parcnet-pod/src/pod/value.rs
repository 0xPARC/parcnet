...
pub enum PodValue {
    ...
    Cryptographic(Fq),
    #[serde(serialize_with = "compressed_pt_ser", deserialize_with = "compressed_pt_de", rename = "eddsa_pubkey")]
    EdDSAPublicKey(Point),
    Boolean(bool),
    Bytes(Vec<u8>),
    #[serde(serialize_with = "milliseconds::serialize", deserialize_with="milliseconds::deserialize")]
    Date(OffsetDateTime),
...
pub fn hash(&self) -> Result<Fq, PodCreationError> {
    let hasher = |input_type, x| Poseidon::new().hash(x).map_err(|e| PodCreationError::HashError(format!("{} hash failed: {}", input_type, e)));
    match self {
        PodValue::String(s) => {
            Ok(string_hash(s))
        }
        PodValue::Int(i) => 
            hasher("Integer", vec![Fq::from(*i)]),
        PodValue::Cryptographic(c) => hasher("Cryptographic", vec![*c]),
        PodValue::EdDSAPublicKey(pt) => hasher("EdDSA public key",
            vec![pt.x, pt.y])
            ,
        PodValue::Boolean(b) => hasher("Boolean", vec![Fq::from(*b)]),
        PodValue::Bytes(b) => Ok(bytes_hash(b)),
        PodValue::Date(t) => hasher("Date", vec![Fq::from(t.unix_timestamp_nanos()/1_000_000)]),
        PodValue::Null => Ok(PrimeField::from_be_bytes_mod_order(&[0;32].iter().map(|_| 0x1d).collect::<Vec<_>>())),
    }
}
}
