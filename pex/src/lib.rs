...
impl PodQueryBuilder {
    ...
    fn add_operation(&mut self, op: Op<StatementRef<'static, 'static>>) {
    ...
    fn convert_statement_ref(&self, sref: StatementRef<'static, 'static>) -> AnchoredKey {
    ...
    match query_oref {
        ORef::Q(_) => {
            if let Some(mapped_sref) = id_mapping.get(&(origin_id, query_key.1.clone())) {
                ...
                Ok(&pod_key.0.origin_name == mapped_sref.0.as_str() && pod_key.1 == mapped_sref.1)
            } else {
